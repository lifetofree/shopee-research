...
- ~~No `SHOPEE_TH_USER_AGENT` field added~~ — **UPDATE 2026-07-16: this bug surfaced in practice and was fixed.** A real `make e2e` run with a genuine cookie failed with `error: 90309999` because `services/search.py`'s hardcoded UA (`Chrome/128.0.0.0`) didn't match the UA of the Playwright Chromium that actually issued the cookie (`Chrome/149.0.0.0` — confirmed via `page.evaluate("() => navigator.userAgent")`, headed launch). `refresh_cookie.py` now captures the real UA and persists it as `SHOPEE_TH_USER_AGENT`; `Settings.user_agent` and `routes.py`'s `search_endpoint` were updated to use it (falling back to the hardcoded default only when empty, e.g. for an anonymous/cold cookie). Regression test added (`test_search_uses_configured_user_agent_not_hardcoded_default`) — verified it fails without the fix and passes with it.
