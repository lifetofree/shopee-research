...
> **Amendment (2026-07-16): Surface B shipped as Surface-A-only, not the merge below.** `capture-affiliate-portal-traffic` confirmed Surface B's real contract (`GET /api/v3/offer/product/list`, REST — not the GraphQL guessed below) but also found it **cannot be replayed server-side**: every request needs `x-sap-sec`/`af-ac-enc-sz-token`/`x-sap-ri`, per-request tokens generated by Shopee's client-side anti-bot SDK inside a real, non-automated browser. Plain `httpx` and even an in-page `fetch()` from a CDP-attached browser both get rejected with `error: 90309999`. **Decision: `services/search.py` ships on Surface A only; `Item.commission` is always `null`.** The section below is kept as the original two-leg design (and the still-accurate Surface A contract) for the record — see `docs/research/data-surfaces.md` §3.2–§3.6 for the full empirical findings and the future-architecture note (a browser extension is the most promising path to real commission data).
...
- **Surface B (affiliate portal)** — **RESOLVED 2026-07-16, fully.** Endpoint is `GET /api/v3/offer/product/list` (REST, `keyword` param confirmed supported), contract in `docs/research/affiliate-observed-traffic.json` / `data-surfaces.md` §3.2–§3.6. Superseding finding: **not server-side replayable** (per-request client-side anti-bot tokens required) — the app ships on Surface A only; `Item.commission` is always `null`. Commission is `seller_commission_rate` → `default_commission_rate` → `max_commission_rate` (string percents) in the confirmed contract, for whenever a non-server-side architecture (browser extension) picks this back up.
